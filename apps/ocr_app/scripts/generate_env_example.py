...
from ocr_app.config.settings import Settings  # noqa: E402
from pydantic import SecretBytes, SecretStr  # noqa: E402
...
def build_env_example(settings_class: type[BaseSettings] = Settings) -> str:
    lines = [
        "# 由 ocr_app.config.settings.Settings 自动生成。",
        "# 请勿在此文件中填写真实密钥。",
        "",
    ]
    ...
    for field_name, field in settings_class.model_fields.items():
        ...
        lines.append(f"{field_name.upper()}={_default_to_env_value(field_name, field)}")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="根据 ocr_app.config.settings.Settings 生成 .env.example。"
    )
